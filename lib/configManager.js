...
*/
  async loadYaml (filePath) {
    try {
      ...
      this.log.debug(`Attempting to load file: ${filePath} from repo: ${params.owner}/${params.repo} ${params.ref ? `ref: ${params.ref}` : '(default branch)'}`)
      
      const response = await this.context.octokit.repos.getContent(params).catch(e => {
        this.log.error(`Error getting settings from ${params.owner}/${params.repo}/${filePath}: ${e.message}`)
      ...
      // If response is null (404 error), return null
      if (!response) {
        this.log.info(`Configuration file not found: ${filePath} in ${params.owner}/${params.repo}`)
        return null
      }
