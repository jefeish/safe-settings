...
module.exports = class Rulesets extends Diffable {
  ...
  update (existing, attrs) {
    // Validate and clean bypass_actors before processing
    if (attrs.bypass_actors) {
      attrs.bypass_actors = this.validateBypassActors(attrs.bypass_actors)
    }
  ...
  validateBypassActors (bypassActors) {
    if (!Array.isArray(bypassActors)) {
      return bypassActors
    }
    
    return bypassActors.filter(actor => {
      // Check if actor_id is valid
      if (typeof actor.actor_id === 'string') {
        // If it's a string, try to convert to number
        const numId = parseInt(actor.actor_id, 10)
        if (isNaN(numId) || actor.actor_id === 'number') {
          this.log.warn(`Filtering out invalid bypass_actor with actor_id: "${actor.actor_id}" (expected integer)`)
          return false
        }
        // Convert string to number
        actor.actor_id = numId
      } else if (typeof actor.actor_id !== 'number') {
        this.log.warn(`Filtering out bypass_actor with invalid actor_id type: ${typeof actor.actor_id}`)
        return false
      }
